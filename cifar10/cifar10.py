...
tf.app.flags.DEFINE_integer('cur_batch_size', 128,
                            """Number of images to process in a batch.""")
tf.app.flags.DEFINE_string('cur_data_dir', '/tmp/cifar10_data',
...
def distorted_inputs():
    ...
    """
    if not FLAGS.cur_data_dir:
        raise ValueError('Please supply a data_dir')
    data_dir = os.path.join(FLAGS.cur_data_dir, 'cifar-10-batches-bin')
    return cifar10_input.distorted_inputs(data_dir=data_dir,
                                          batch_size=FLAGS.cur_batch_size)
...
def inference(images):
    ...
    with tf.variable_scope('local3') as scope:
        ...
        for d in pool2.get_shape()[1:].as_list():
            dim *= d
        reshape = tf.reshape(pool2, [FLAGS.cur_batch_size, dim])
...
def train(total_loss, global_step):
    ...
    num_batches_per_epoch = NUM_EXAMPLES_PER_EPOCH_FOR_TRAIN / FLAGS.cur_batch_size
